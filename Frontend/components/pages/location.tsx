...
import useLocationForm from 'components/location/useLocationForm';
import { setLocationName } from 'store/slices/location.slice';
...
const LocationPage = () => {
  ...
  const { create, delete: deleteOne, update } = useLocationForm();
  const [name, setName] = useState<string>("");
  const [curId, setCurId] = useState(-1);
  const { account } = useAppSelector(state => state.auth);

  const getAllLocation: any = useQuery(
    ['getAllLocation'],
    () => {
      return http.get('/location/read')
    }
  )

  const allCategory = React.useMemo(() => {
    return getAllLocation.data ?
      getAllLocation.data.data : []
  }, [getAllLocation.data])
          ...
          allCategory?.map((each: any, index: number) => (
            <div key={index} className='bg-blue-500 hover:bg-green-500 transition-all duration-300 ease-in-out text-white rounded-md flex justify-between items-center hover:cursor-pointer'>
              <div className='pl-8 pr-8 py-1' onClick={() => {
                dispatch(setLocationName(each.name))
                router.push(`/location/${each.id}`)
              }}>
                {each.name}
              </div>
