...
import useItemForm from 'components/cloud-words/useItemForm';
...
const ItemPage = () => {
  ...
  const { categoryName, subcategoryName } = useAppSelector(state => state.cws);
  const [name, setName] = useState<string>("");
  const [curId, setCurId] = useState(-1);
  const { account } = useAppSelector(state => state.auth);

  const getAllItem: any = useQuery(
    ['getAllItem', subId],
    () => {
      if (subId)
        return http.get(`/item/read?id=${subId}`)
    }
  )

  const allItem = React.useMemo(() => {
    return getAllItem.data ?
    getAllItem.data.data : []
  }, [getAllItem.data])
  ...
  return (
    <div className='px-12 py-16 space-y-12'>
      <div className='text-2xl'>
        <span className='hover:cursor-pointer hover:text-red-500' onClick={() => router.push(`/category/${id}`)}>{categoryName}</span>
        <span> / </span>
        <span className='hover:cursor-pointer hover:text-red-500'>{subcategoryName}</span>
      </div>
      ...
      <div className='flex flex-wrap gap-4'>
        {
          allItem?.map((each: any, index: number) => (
